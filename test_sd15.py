...
def parse_args():
    ...
    parser.add_argument(
        "--pretrain_path", type=str, default='stable-diffusion-v1-5/stable-diffusion-v1-5',
    )
...
def load_all_model(model_path, args, dtype=torch.bfloat16):
    ...
    text_encoder = CLIPTextModel.from_pretrained(pretrain_path, subfolder="text_encoder", torch_dtype=dtype)
    ...
    vae = AutoencoderKL.from_pretrained(pretrain_path, subfolder="vae", torch_dtype=torch.float32)

    print("Init SD1.5 model")
    ...
    return text_encoder, scheduler, tokenizer, vae, unet
...
def pipe(components, prompts, height, width, num_steps, seed, cfg=1.0, device='cuda'):
    ...
    text_encoder, scheduler, tokenizer, vae, unet = components

    prompt_embeds = encode_text(tokenizer, text_encoder, prompts)
    negative_prompt_embeds = encode_text(tokenizer, text_encoder, "")
    ...
    with torch.no_grad():
        for i, t in enumerate(timesteps):
            ...
            pred = unet(latent_model_input, t, encoder_hidden_states=torch.cat([prompt_embeds, negative_prompt_embeds]), return_dict=False)[0]
