...
# from utils.tokenizer_hps import HFTokenizer  # not needed for CLIP path
...
class Selector:
    ...
    def score_pairs(self, img_paths_a: List[str], img_paths_b: List[str], prompts: List[str],
        ...
        """
        Computes CLIP similarity for (prompt_i, imgA_i) and (prompt_i, imgB_i) in batches.
        ...
        for start in tqdm(range(0, n, batch_size), desc="Scoring (CLIP)", leave=False):
            ...
            diag = torch.diagonal(logits_per_image)              # [2B]
...
def main():
    parser = argparse.ArgumentParser(description="Pairwise CLIP win-rate evaluation")
    ...
    print(f"Mean CLIP -> {label_a}: {mean_a:.6f} | {label_b}: {mean_b:.6f}")
    ...
    if args.save is None:
        ...
        # Keep filename suffix identical for downstream parsers
    ...
    summary = {
        ...
        "mean_score_a": mean_a,       # field name kept for compatibility (actually CLIP mean)
        "mean_score_b": mean_b,
