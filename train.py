...
def main():
    ...
    if args.train_method != 'sft': ref_unet.requires_grad_(False)
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process:
                torch.save(unwrap_model(unet).state_dict(), os.path.join(output_dir, "unet.pth"))
        ...
        def load_model_hook(models, input_dir):
            ...
            if os.path.exists(os.path.join(input_dir, "unet.pth")):
                state_dict = torch.load(os.path.join(input_dir, "unet.pth"))
            else:
    ...
    if ('pickapic' in args.dataset_name) or (args.train_method != 'sft'):
        pass
